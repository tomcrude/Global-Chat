...
export default function Register() {
  ...
  return (
    ...
    <div className={event === 1 ? "screamer" : "inactive"}>
    </div>
    
    <audio src={audio} className="audiox"></audio>

    <div className={ event === 2 ? "container bomb" : "container mt-5"}>

    <div className="row">
     <a onClick={()=>{navigate("/")}} className="text-center text-decoration-none change">Change your name</a>
     </div>
      ...
      <div className="mt-4 border border-dark chat pt-3">
      {
        mensaje.map((stat)=>{
          ...
          if( stat.message === "//screamer"){setTimeout (async()=>{
          ...
          sound2.play(); setevent(2)},200); setTimeout (()=>{setevent(0)},3000) ;}
            ...
            <div className="row break-w" key={stat.id}>
